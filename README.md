# Governance Policies Terraform

This repository is ready to run Terraform CI/CD from GitHub Enterprise Cloud.
...
3. Provide Terraform input values either by committing a reviewed `infra\prod.auto.tfvars` copied from `infra\terraform.tfvars.example`, or by storing the full tfvars content in the GitHub Actions secret `TERRAFORM_TFVARS`.
...
5. Push to GHE. `Terraform CI` formats and validates; `Terraform CD` logs in with OIDC, runs a plan, uploads the plan artifact, and applies from `main`.
